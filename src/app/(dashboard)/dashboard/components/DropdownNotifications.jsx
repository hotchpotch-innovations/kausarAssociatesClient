...
function DropdownNotifications({
  ...
  return (
    <div className="relative inline-flex">
      <button
        ref={trigger}
        className={`w-8 h-8 flex items-center justify-center hover:bg-green-200  dark:bg-slate-700 dark:hover:bg-slate-600/80 rounded-full ${dropdownOpen ? 'bg-green-300' : 'bg-slate-100'}`}
        aria-haspopup="true"
      ...
      <Transition
        className={`origin-top-right z-10 absolute top-full -mr-48 sm:mr-0 w-80 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 py-1.5 rounded shadow-lg overflow-hidden mt-1 ${align === 'right' ? 'right-0' : 'left-0'}`}
        show={dropdownOpen}
