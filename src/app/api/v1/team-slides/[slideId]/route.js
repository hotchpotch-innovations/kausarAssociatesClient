...
export async function PATCH(request, content) {
  try {
    ...
    const result = await TeamSlide.findByIdAndUpdate(slideId, payload);
...
export async function DELETE(request, content) {
  ...
  try {
    await mongoose.connect(connectionStr);
    const result = await TeamSlide.findByIdAndDelete(slideId);
    return NextResponse.json({
      success: true,
...
export async function GET(request, content) {
  ...
  try {
    await mongoose.connect(connectionStr);
    const result = await TeamSlide.findById(filter);
    return NextResponse.json({
      success: true,
      data: result,
    });
